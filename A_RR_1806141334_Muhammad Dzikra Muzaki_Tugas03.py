    ...
    def generate_barcode(self, event):
        ...
        for chr in reserved_chars:
            if chr in filename:
                ...
                error_message += "FilenameError: Cannot use reserved characters. Please input a filename without /, \\, ?, %, *, :, |, \", <, >."
                break
        ...
        if filename[-4:] != ".eps":
            ...
            if error_message != "":
                error_message += "\n\n"
            error_message += "FilenameError: File extension not valid. Please input a .eps filename."

        if not (len(barcode) == 12 and barcode.isnumeric()):
            barcode_invalid == True
            if error_message != "":
                error_message += "\n\n"
            error_message += "BarcodeError: Code number length is not 12 or not an integer."
        ...
        if filename_invalid or barcode_invalid:
            ...
            self.barcode_canvas.create_barcode(filename, barcode)
...
class Barcode_Generator(Canvas):
    ...
    def draw_lines(self, x, y, codes, side, color="black"):
        for i in range(len(codes)):
            if (codes[i] == '1'):
                self.draw_canvas.create_line(x+2*i, y, x+2*i, y + side, width=2, fill=color)
			# else:
			# 	self.create_line(x+2*i, y, x+2*i, y + side, width=2, fill="yellow") # FOR DEBUG PURPOSES
    ...
    def create_barcode(self, filename, barcode):
        ...
        self.draw_lines(poes_x + 20, pos_y + 60, "101", 100, "blue")
